...
A third patch on an unexplained base is the most reliable way to turn a one-hour bug into a session. If the clean restart also stalls, hand the repro and the dead hypotheses to one fresh agent with a written brief and have it re-run the narrowing independently — independent re-checking by a separate agent is the countermeasure with the best measured effect on false positives, and it costs 3–10× the tokens, so do it once rather than as a habit. Where running and narrowing this failure is inside the recorded worker's envelope, it gets that brief, announce line included (Law 11, [worker.md](worker.md)); what comes back is a hypothesis with a reproduction, never a verdict — you re-run the reproduction here.
